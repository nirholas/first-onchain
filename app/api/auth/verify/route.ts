...
export async function POST(request: NextRequest) {
  try {
    ...
    const key = new PublicKey(body.address); const message = new TextEncoder().encode(authMessage(body.address, nonce, request.nextUrl.host));
    const signature = bs58.decode(body.signature);
    if (signature.byteLength !== nacl.sign.signatureLength || !nacl.sign.detached.verify(message, signature, key.toBytes())) return NextResponse.json({ error: "Invalid signature" }, { status: 401 });
