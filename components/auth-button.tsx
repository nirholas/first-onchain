...
export function AuthButton(){
  ...
  return <button className="button secondary small" disabled={busy} onClick={async()=>{try{setBusy(true);const {nonce}=await fetch("/api/auth/nonce",{cache:"no-store"}).then(r=>r.json());const message=authMessage(address,nonce,window.location.host);const signature=await client.wallet.signMessage(new TextEncoder().encode(message));const response=await fetch("/api/auth/verify",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({address,signature:bs58.encode(signature)})});if(!response.ok)throw new Error("Sign-in failed");setSession(address)}finally{setBusy(false)}}}>{busy?"Signing…":"Sign in"}</button>;
}
