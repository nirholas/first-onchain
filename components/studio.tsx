...
function AgentPanel() {
  const [form,setForm]=useState({name:"",description:"",endpoint:"",capabilities:"research, execute",version:"1.0.0"}); const [notice,setNotice]=useState<Notice>(null);const[busy,setBusy]=useState(false); const {publish,connected,supportsV1}=useInscribe();
...
function VanityPanel(){
  ...
  const run=()=>{if(!/^[1-9A-HJ-NP-Za-km-z]*$/.test(prefix+suffix)){setNotice({kind:"error",message:"Use Base58 characters only (no 0, O, I, or l)."});return}if(prefix.length+suffix.length>6){setNotice({kind:"error",message:"Use at most 6 combined prefix and suffix characters for a practical browser search."});return}worker.current?.terminate();const next=new Worker("/vanity-worker.js");worker.current=next;setRunning(true);setResult(null);setAttempts(0);setNotice(null);const fail=(message:string)=>{setNotice({kind:"error",message});setRunning(false);next.terminate();worker.current=null};next.onerror=()=>fail("The vanity worker stopped unexpectedly.");next.onmessage=event=>{if(event.data.type==="progress")setAttempts(event.data.attempts);if(event.data.type==="found"){setAttempts(event.data.attempts);setResult({address:event.data.address,secretKey:new Uint8Array(event.data.secretKey)});setRunning(false);next.terminate();worker.current=null}if(event.data.type==="error")fail(event.data.message)};next.postMessage({type:"start",prefix,suffix,caseSensitive})};
  ...
  return <section className="panel"><div className="panel-head"><div><h2>Vanity address</h2><p>Search for a memorable Solana keypair in an isolated browser worker. Nothing is transmitted or saved by First.</p></div><Fingerprint size={30}/></div><div className="form-grid"><div className="field"><label>Starts with</label><input value={prefix} onChange={e=>setPrefix(e.target.value.slice(0,6))} placeholder="first"/></div><div className="field"><label>Ends with</label><input value={suffix} onChange={e=>setSuffix(e.target.value.slice(0,6))} placeholder="sol"/></div><div className="field full"><label><input style={{width:"auto",marginRight:8}} type="checkbox" checked={caseSensitive} onChange={e=>setCaseSensitive(e.target.checked)}/> Case-sensitive</label><small>Estimated search space: {difficulty.toLocaleString()} attempts. Use at most 6 combined prefix and suffix characters.</small></div></div>{running&&<div className="status">Searching off the main thread… {attempts.toLocaleString()} attempts</div>}{result&&<><div className="status success">Match found after {attempts.toLocaleString()} attempts.</div><div className="vanity-result">{result.address}</div></>}<NoticeBox notice={notice}/><div className="panel-actions"><small>Never share the downloaded secret key.</small><div style={{display:"flex",gap:8}}>{result&&<button className="button secondary" onClick={download}>Download keypair</button>}{running?<button className="button" onClick={stop}>Stop search</button>:<button className="button" disabled={!prefix&&!suffix} onClick={run}>Generate locally</button>}</div></div></section>;
}
